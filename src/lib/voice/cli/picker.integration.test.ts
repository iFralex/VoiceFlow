...
async function seedSharedPool(tx: TestDbTx) {
  // The picker excludes rows where provider_external_id IS NULL — the seed
  // pool ships with NULLs because the founder populates each Vapi phoneNumberId
  // post-procurement. Tests inject placeholder values so the filter passes.
  await tx.insert(phoneNumbers).values([
    {
      ...
      org_id: null,
      provider: 'voiped',
      provider_external_id: 'vapi-phone-milano',
      status: 'active',
      region: 'milano',
    ...
    },
    {
      ...
      org_id: null,
      provider: 'voiped',
      provider_external_id: 'vapi-phone-roma',
      status: 'active',
      region: 'roma',
      capabilities: ['landline'],
      daily_call_count: 0,
    ...
    },
    {
      ...
      org_id: null,
      provider: 'voiped',
      provider_external_id: 'vapi-phone-napoli',
      status: 'active',
      region: 'napoli',
      capabilities: ['landline'],
      daily_call_count: 0,
    ...
    },
    {
      ...
      org_id: null,
      provider: 'voiped',
      provider_external_id: 'vapi-phone-mobile',
      status: 'active',
      region: null,
      capabilities: ['mobile'],
      daily_call_count: 0,
  ...
  it.skipIf(skipWhenNoDb)('prefers an org-dedicated CLI over the shared pool', async () => {
    await withTestDb(async (tx) => {
      ...
      await tx.insert(phoneNumbers).values({
        ...
        org_id: ORG_A,
        provider: 'voiped',
        provider_external_id: 'vapi-phone-dedicated-a',
        status: 'active',
        region: 'milano',
        capabilities: ['landline'],
  ...
  it.skipIf(skipWhenNoDb)("does not return another org's dedicated CLI", async () => {
    await withTestDb(async (tx) => {
      ...
      await tx.insert(phoneNumbers).values({
        ...
        org_id: ORG_B,
        provider: 'voiped',
        provider_external_id: 'vapi-phone-dedicated-b',
        status: 'active',
        region: 'milano',
        capabilities: ['landline'],
        daily_call_count: 0,
  ...
  it.skipIf(skipWhenNoDb)('excludes CLIs at the daily cap', async () => {
    await withTestDb(async (tx) => {
      ...
      await tx.insert(phoneNumbers).values({
        ...
        org_id: null,
        provider: 'voiped',
        provider_external_id: 'vapi-phone-at-cap',
        status: 'active',
        region: 'milano',
        capabilities: ['landline'],
  ...
  it.skipIf(skipWhenNoDb)('skips cooling_down CLIs', async () => {
    await withTestDb(async (tx) => {
      ...
      await tx.insert(phoneNumbers).values({
        ...
        org_id: null,
        provider: 'voiped',
        provider_external_id: 'vapi-phone-cooling',
        status: 'cooling_down',
        region: 'milano',
        capabilities: ['landline'],
        daily_call_count: 0,
        ...
        await tx.insert(phoneNumbers).values([
          {
            ...
            org_id: null,
            provider: 'voiped',
            provider_external_id: 'vapi-phone-recent',
            status: 'active',
            region: 'milano',
            capabilities: ['landline'],
            daily_call_count: 0,
          ...
          },
          {
            ...
            org_id: null,
            provider: 'voiped',
            provider_external_id: 'vapi-phone-idle',
            status: 'active',
            region: 'milano',
            capabilities: ['landline'],
            daily_call_count: 0,
        ...
        await tx.insert(phoneNumbers).values([
          {
            ...
            org_id: null,
            provider: 'voiped',
            provider_external_id: 'vapi-phone-r10',
            status: 'active',
            region: 'milano',
            capabilities: ['landline'],
            daily_call_count: 0,
          ...
          },
          {
            ...
            org_id: null,
            provider: 'voiped',
            provider_external_id: 'vapi-phone-r20',
            status: 'active',
            region: 'milano',
            capabilities: ['landline'],
            daily_call_count: 0,
