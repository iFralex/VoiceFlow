...
const SKIP = process.env['SKIP_E2E_LAUNCH_SMOKE'] !== 'false' || !process.env['E2E_TEST_PHONE'];
...
async function uploadContactsCsv(
  ...
  await expect(page.getByText(new RegExp(`${expectedCount}\\s+contatt`, 'i'))).toBeVisible({
    timeout: 60_000,
  });
...
async function pollCampaignUntilDone(
  ...
  while (Date.now() < deadline) {
    ...
    const completata = await page
      .getByText('Completata')
      .isVisible()
      .catch(() => false);
    const annullata = await page
      .getByText('Annullata')
      .isVisible()
      .catch(() => false);
...
test.describe('Launch smoke test — full customer journey (staging only)', () => {
  test.skip(SKIP, 'Skipped: set SKIP_E2E_LAUNCH_SMOKE=false and E2E_TEST_PHONE=+39... to enable');
  ...
  test('sign up → onboarding (DPA) → top-up → contacts upload → script → campaign → results → GDPR', async ({
    browser,
  }) => {
    ...
    try {
      ...
      await expect(
        page.getByText(/completat[ao]|segreteria|senza risposta|fallita|occupato/i).first(),
      ...
      await page.fill('#erase-reason', 'Richiesta diritto alla cancellazione — test smoke E2E');
